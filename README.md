...
Для разработки интерфейса и прототипирования сайта была использована "Figma". Макет проекта представлен на рисунке 2. 
...
При  переходе  на  сайт  происходит  отображение  главной  страницы, реализация  которой  находится  внутри  main,  данная  страница  является  связующим  звеном  между  всеми  частями  сайта  с  встроенным  меню  и различными возможностями отображения других страниц. 
...
Код, отвечающий за работу записи, прописан в приложениях appointment. Параметры  записи:  имя,  номер  телефона,  почта  и  т.д.  вводятся пользователем на соответствующей странице и передаются в БД. В панели администратора можно посмотреть пользовательские записи.  
